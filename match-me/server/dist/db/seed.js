...
const seed = async () => {
    try {
        ...
        await index_1.default.query('TRUNCATE users, profiles, connections, messages RESTART IDENTITY CASCADE');
        ...
        for (const user of users) {
            const userRes = await index_1.default.query('INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id', [user.email, user.passwordHash]);
            const userId = userRes.rows[0].id;
            await index_1.default.query(`INSERT INTO profiles (user_id, first_name, last_name, bio, interests, location, gender, avatar_url)
